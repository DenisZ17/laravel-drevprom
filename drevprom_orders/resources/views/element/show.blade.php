<x-app-layout title="Карточка детали">
    <div class="w-full mt-4">
        <div class="max-w-sm  border  rounded-lg shadow bg-gray-800 border-gray-700 mx-auto">
            <a href="#">
                <img class="rounded-t-lg" src="{{ asset('uploads/' . $element->image) }}" alt="" />
            </a>
            <div class="p-5">
                <a href="#">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight  text-white">{{$element->title}}</h5>
                </a>
                <p class="mb-3 font-normal  text-gray-400">{{(Str::length($element->description)==0)? "Описание отсутствует" : $element->description}}</p>
                <p class="mb-3 font-normal text-sm text-yellow-300">{{(Str::length($element->info)==0)? "Доп. информация отсутствует" : $element->info}}</p>
                <a wire:navigate href="{{route('knowledge')}}"
